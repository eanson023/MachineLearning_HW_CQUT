...
def load_iris_data(train_proportion=0.7, visualize=True):
    from sklearn.datasets import load_iris
    X_pd, y_pd = load_iris(return_X_y=True, as_frame=True)
    if visualize:
        viz.visualize_iris(np.asarray(X_pd), np.asarray(y_pd))
    # shuffle一下
    data_pd = shuffle(pd.concat([X_pd, y_pd], axis=1))
    # 训练集按照比例划分
    m = math.floor(len(data_pd)*train_proportion)
    # 按照比例读取训练集和验证集
    X_train, y_train = np.asarray(
        data_pd.iloc[:m, :3]), np.asarray(data_pd.iloc[:m, -1])
    X_cv, y_cv = np.asarray(data_pd.iloc[m:, :3]), np.asarray(
        data_pd.iloc[m:, -1])
    return X_train, y_train, X_cv, y_cv
